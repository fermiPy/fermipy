...
class LightCurve(object):
    ...
    def LC(self, name, **kwargs): 

        """Generate a lightcurve for the named source. The function will complete the basic analysis steps 
        for each bin and perform a likelihood fit for each bin. Extracted values (along with errors) are 
        Integral Flux, spectral model, Spectral index, TS value, pred. # of photons.
        ...
        calc_ul: bool
             specifies whether or not to calculate upper limits for flux points below a threshold TS

        thresh_TS: float
            threshold values of TS below which triggers UL calculation (if calc_ul is true)
        ...
        config = copy.deepcopy(self.config['LC'])
        ...
        filename = utils.format_filename(self.workdir, 'LC',
...
#add in columns for model parameters
     ...
     hdulist = pyfits.HDUList([hdulist[0], hdulist[1]])
...
#main method
    def _make_LC(self, name, **config):
        ...
        self.times = np.arange(self.config['selection']['tmin'],self.config['selection']['tmax'],binning)
...
#run gtselect
                kw_gtselect = dict(infile=self.config['data']['evfile'],
                           ...
                           rad='INDEF',#self.config['selection']['radius'],
...
#run gtbin
                ...
                kw_gtccube = dict(algorithm='CCUBE',
                                  ...
                                  scfile=self.config['data']['scfile'],
                                  nxpix=np.int((np.float(self.config['binning']['roiwidth'])*np.sqrt(2)/0.2)),
                                  nypix=np.int((np.float(self.config['binning']['roiwidth'])*np.sqrt(2)/0.2)),
                                  binsz=0.2,
                                  coordsys='CEL',
                ...
                if (not path.exists('LC_ccube_binned'+str(i)+'.fits')):
                    ...
                    fermipy.gtanalysis.run_gtapp('gtbin', self.logger, kw_gtccube)
                else:
...
#run gtexpcube2                                   
                kw_expcube2 = dict(infile='LC_ltcube_bin'+str(i)+'.fits',
                                   ...
                                   irfs='P8R2_SOURCE_V6',
                                   nxpix=np.int(((np.float(self.config['binning']['roiwidth'])+10.0)*np.sqrt(2)/0.2)),
                                   nypix=np.int(((np.float(self.config['binning']['roiwidth'])+10.0)*np.sqrt(2)/0.2)),
                                   binsz=0.2,
                                   coordsys='CEL',
                                   xref=self.roi.skydir.ra.deg,
                                   yref=self.roi.skydir.dec.deg,
                ...
                if (not path.exists('LC_expcube_bin'+str(i)+'.fits')):
                    ...
                    fermipy.gtanalysis.run_gtapp('gtexpcube2', self.logger, kw_expcube2)
                else:
...
#run gtsrcmaps
                ...
                if (not path.exists('LC_srcmap_bin'+str(i)+'.fits')):
                    ...
                    fermipy.gtanalysis.run_gtapp('gtsrcmaps', self.logger, kw_srcmap)
                else:
...
#
#
                srcfreepar = binnedA.freePars(str(self.config['selection']['target']))
                srcnormpar=binnedA.normPar(self.config['selection']['target'])

                if len(srcfreepar)>0:
                    binnedA.setFreeFlag(str(self.config['selection']['target']), srcfreepar, 0)
                    binnedA.syncSrcParams(str(self.config['selection']['target']))
                                           
                self.logger.info('Computing LL for all source parameters fixed')
                ...
                if (likeObj.getRetCode() != 0):
                    self.logger.info('Looks like the fit didnt converge, lets try deleting some low (<1) TS sources')
                    deletesrc = []
                ...
                if (likeObj.getRetCode() != 0):
                    self.logger.info('Looks like the fit still didnt converge, lets try deleting some more low (<2) TS sources')
                    deletesrc = []
                    for s in like.sourceNames():
                        freepars = binnedA.freePars(s)
                        if(s != str(self.config['selection']['target']) and binnedA[s].src.getType() == 'Point' and len(freepars)>0):
                    ...
                    self.logger.info('Last time trying the fit....if it still doesnt converge you might have to tinker with your model more for this bin....')
                ...
                if (likeObj.getRetCode() != 0):
                    self.logger.info('Warning- the fit still didnt converge....')
...
#                print binnedA.flux(str(self.config['selection']['target']),  emin=self.config['selection']['emin'], emax=self.config['selection']['emax'])
                ...
                binnedA.setFreeFlag(str(self.config['selection']['target']), srcfreepar, 1)
                binnedA.syncSrcParams(str(self.config['selection']['target']))
...
#make the output dictionary only once (have to do it inside loop since it pulls vals from the like object

                
                                                                                                                                                                        
                if (i == 0):
                    o =collections.OrderedDict()
                    o.update({'name': name, 'plottimes':  np.zeros_like(self.times[:-1]),                                                                                        
                    'model': [],                                                                                                                                             
                    ...
                    for pn in binnedA[str(self.config['selection']['target'])].funcs['Spectrum'].paramNames:                    
                        ...
                        o.update({'par_'+str(p.getName())+'_err': np.zeros_like(self.times[:-1]), 'par_'+str(p.getName()): np.zeros_like(self.times[:-1])})
                    o.update({'retCode':  np.zeros_like(self.times[:-1]),'npred':  np.zeros_like(self.times[:-1])})    
        #Fill up the dict:


                for pn in binnedA[str(self.config['selection']['target'])].funcs['Spectrum'].paramNames:
                    p = binnedA[str(self.config['selection']['target'])].funcs['Spectrum'].getParam(pn)
                    o['par_'+p.getName()+'_err'][i] = p.error()*p.getScale()
                    o['par_'+p.getName()][i] = p.getTrueValue()

                    

                    
                o['model'].append(self.roi[self.config['selection']['target']]['SpectrumType'])    
                o['IntFlux'][i] = binnedA.flux(str(self.config['selection']['target']),  emin=self.config['selection']['emin'], emax=self.config['selection']['emax'])    
                o['IntFluxErr'][i] = binnedA.fluxError(str(self.config['selection']['target']),  emin=self.config['selection']['emin'], emax=self.config['selection']['emax'])
...
#run gtdiffrsp
                ...
                self.IntFlux[i] = UnbinnedA.flux(str(self.config['selection']['target']),  emin=self.config['selection']['emin'], emax=self.config['selection']['emax'])
                self.IntFluxErr[i] = UnbinnedA.fluxError(str(self.config['selection']['target']),  emin=self.config['selection']['emin'], emax=self.config['selection']['emax'])
