...
model = {
    'src_radius'        :
        (None,'Set the maximum distance for inclusion of sources in the ROI '
         ...
         'will be ORed with sources passing the cut on src_roiwidth.',float),
    'src_roiwidth'        :
...
binning = {
    ...
    'coordsys'   : ('CEL','',str),        
    'npix'       : 
    (None, 'Number of spatial bins.  If none this will be inferred from roiwidth '
     'and binsz.',int), 
    'roiwidth'  : (10.0,'Set the width of the ROI in degrees.  If both roiwidth and binsz are '
                    'given the roiwidth will be rounded up to be a multiple of binsz.',float),
...
fileio = {
    ...
    'usescratch'   : (False,'Perform analysis in a temporary working directory.',bool),
    }
