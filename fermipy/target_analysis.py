...
class AnalyzeROI(Link):
    ...
    def run_analysis(self, argv):
        ...
        gta.optimize()
        gta.print_model()
        gta.delete_sources(minmax_ts=[None,25.],exclude=['isodiff','galdiff00','galdiff01','galdiff02',gta.roi.sources[0].name])
        #gta.delete_sources(minmax_ts=[None,25.],exclude=['isodiff','galdiff00',gta.roi.sources[0].name])
            ...
            if extension == 'lower' or extension == 'upper':
                if TSext_gauss>TSext_disk and TSext_gauss>=9.0:
                    ...
                    if extension == 'lower':
                        ...
                        'SpectrumType' : 'PowerLaw', 'Index' : 2.0,
                        'Scale' : 1000, 'Prefactor' : 1e-11,
                        'SpatialModel': 'RadialGaussian', 'SpatialWidth': size_gauss-size_gauss_lo })
                elif TSext_gauss<=TSext_disk and TSext_gauss>=9.0:
                    if extension == 'upper':
                        gta.add_source(namesource,{ 'glon' : glon0, 'glat' : glat0,
                        'SpectrumType' : 'PowerLaw', 'Index' : 2.0,
                    ...
                    if extension == 'lower':
                        ...
                        'SpectrumType' : 'PowerLaw', 'Index' : 2.0,
                        'Scale' : 1000, 'Prefactor' : 1e-11,
                        'SpatialModel': 'RadialDisk', 'SpatialWidth': size_disk-size_disk_lo })	
                else:
                    gta.load_roi('roi_pointlike')
                print('Spatial Template: ',gta.roi.sources[0]['SpatialType'],gta.roi.sources[0]['SpatialWidth'])
