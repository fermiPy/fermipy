...
from fermipy.stack.stack_spectral import ConvertCastro_SG, StackLikelihood_SG
from fermipy.stack.stack_collect import CollectLimits_SG, CollectStackedLimits_SG
from fermipy.stack.stack_plotting import PlotStack_SG, PlotLimits_SG,\
    PlotStackedStack_SG, PlotStackedLimits_SG, PlotControlLimits_SG,\
    PlotFinalLimits_SG, PlotControlMLEs_SG
from fermipy.stack.stack_prepare import PrepareTargets
from fermipy.stack.stack_spectral import SpecTable

from fermipy.stack import defaults


def _get_plot_config(plotting_dict, key,
                     plot_specs_default=None, astro_priors_default=None):
    """ Get the configuration for a type of plot """
    sub_dict = plotting_dict.get(key, None)
    if sub_dict is None:
        return None
    specs = sub_dict.get('plot_specs', plot_specs_default)
    astro_priors = sub_dict.get('astro_priors', astro_priors_default)
    return purge_dict(dict(specs=specs,
                           astro_priors=astro_priors))


class PipelineData(Chain):
    """Chain together the steps of the stacking pipeline
    ...
    convert-castro : `ConvertCastro_SG`
        Convert the SED to stacking-space for each target, profile and norm prior type
    ...
    plot-stack : `PlotStack_SG`
        Make Stacking 'Castro' plots for each profile, target, norm prior type and spec.

    plot-limits : `PlotLimits_SG`
        Make Stack 'Castro' plots for each profile, target, norm prior type and spec.

    plot-stacked-stack : `PlotStackedStack_SG`
        Make Stacking 'Castro' plots for each roster, norm prior type and spec.
    ...
    def _map_arguments(self, args):
        ...
        plot_specs_default = config_dict.get('plot_specs', [])
        ...
        config_plot_stack = _get_plot_config(data_plotting, 'plot-stack',
                                             plot_specs_default, astro_priors)
        if config_plot_castro is not None:
            self._set_link('plot-stack-sg',
                           PlotStack_SG,
                           ttype=ttype,
                           targetlist=targetlist,
                           **config_plot_stack)
        ...
        config_plot_stacked_stack = _get_plot_config(data_plotting, 'plot-stacked-stack',
                                                     plot_specs_default, astro_priors)
        if config_plot_stacked_stack is not None:
            self._set_link('plot-stacked-stack-sg',
                           PlotStackedStack_SG,
                           ttype=ttype,
                           rosterlist=rosterlist,
                           **config_plot_stacked_stack)
...
class PipelineSim(Chain):
    ...
    plot-control-mles : `PlotControlMLEs_SG`
        Make stacking Maximum Likelihood estimate plots for each roster,
        J-factor prior type and spec.

    """
    appname = 'stack-pipeline-sim'
    ...
    def _map_arguments(self, args):
        ...
        config_plot_stacked_stack = _get_plot_config(sim_plotting, 'plot-stacked-stack',
                                                     plot_specs_default, astro_priors)
        if config_plot_stacked_stack is not None:
            self._set_link('plot-stacked-stack',
                           PlotStackedStack_SG,
                           ttype=ttype,
                           sim=sim_name,
                           rosterlist=rosterlist,
...
class PipelineRandom(Chain):
    ...
    convert-castro : `ConvertCastro_SG`
        Convert the SED to stacking-space for each target, profile and J-factor prior type
    ...
    def _map_arguments(self, args):
        ...
        config_plot_stacked_stack = _get_plot_config(rand_plotting, 'plot-stacked-stack',
                                                     plot_specs_default, astro_priors)
        if config_plot_stacked_stack is not None:
            self._set_link('plot-stacked-stack',
                           PlotStackedStack_SG,
...
class Pipeline(Chain):
    """Top level stacking pipeline analysis chain.
    ...
    spec-table : `SpecTable`
        Build the FITS table with the stacking spectra for all the spectra
        being analyzed.
    ...
    def _map_arguments(self, args):
        ...
        plot_specs = config_dict.get('plot_specs', [])
