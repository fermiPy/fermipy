...
def compute_ps_counts(ebins, exp, psf, bkg, fn, egy_dim=0):
    ...
    theta_edges = np.linspace(0.0,3.0,31)[np.newaxis,:]*np.ones((len(ectr),31))
    ...
    sig_pdf = domega*psf.interp(ectr[:,np.newaxis], theta)*(np.pi/180.)**2
    sig_flux = fn.flux(ebins[:-1],ebins[1:])
    
    # Background and signal counts
    bkgc = bkg[:,np.newaxis]*domega*exp[:,np.newaxis]*ewidth[:,np.newaxis]*(np.pi/180.)**2
...
class Exposure(HpxMap):
    ...
    def create(ltc, event_class, event_types, log_energies):
        ...
        for et in event_types:
            ...
            exp += np.sum(aeff.T[:,:,np.newaxis]*ltc.data[:,np.newaxis,:],axis=0)

        hpx = HPX(ltc.hpx.nside,ltc.hpx.nest,ltc.hpx.coordsys,ebins=log_energies)
