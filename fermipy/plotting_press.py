...
def make_counts_spectrum_plot(o, roi, energies, imfile, **kwargs):
    ...
    ax0.set_ylim(0.1, 4e6)
    ax0.set_xlim(energies[0], energies[-1])
    ax0.legend(frameon=False, loc='best', prop={'size': 12}, ncol=2)
...
class AnalysisPlotter(fermipy.config.Configurable):
    ...
    def make_residmap_plots(self, maps, roi=None, **kwargs):
        ...
        p.plot(vmin=-2, vmax=2, levels=sigma_levels,
	...
	cmap_resid = 'hot_r'
        ...
        p.plot(cb_label='Counts', interpolation='gaussian',
               cmap=cmap, zscale='sqrt', vmin=0.1, vmax=80.)
	...
	plt.savefig(utils.format_filename(workdir,
        ...
        fig = plt.figure(figsize=(8,6))
        maps['model'].data = gaussian_filter(maps['model'].data, 4.0)
        p = ROIPlotter(maps['model'], roi=roi, **kwargs)
        p.plot(cb_label='Counts', interpolation='gaussian',
        #       cmap=cmap, zscale='sqrt', vmin=vmin, vmax=vmax)
               cmap=cmap, zscale='linear', vmin=0., vmax=0.5)
        fig.tight_layout(pad=3.0)
        plt.savefig(utils.format_filename(workdir,
                                          'residmap_model',
                                          prefix=[prefix],
	...
	plt.savefig(utils.format_filename(workdir,
        ...
        fig = plt.figure(figsize=(8,6))
        #print('map',maps['excess'],maps['excess'].data,help(maps['excess']))
        maps['excess'].data = gaussian_filter(maps['excess'].data, 4.0)
        p = ROIPlotter(maps['excess'], roi=roi, **kwargs)
        #p.plot(cb_label='Counts', interpolation='bicubic',
        #       cmap=cmap_resid, vmin=vmin, vmax=vmax)
        #print('Type',type(maps['excess']),type(gaussian_filter(maps['excess'], 3.0)))
        #p = ROIPlotter(gaussian_filter(maps['excess'], 3.0), roi=roi, **kwargs)
        p.plot(cb_label='Counts', interpolation='gaussian',
        #       cmap=cmap_resid, vmin=vmin, vmax=vmax)
               cmap=cmap_resid, zscale='linear', vmin=-1.0, vmax=1.0)
        fig.tight_layout(pad=3.0)
        plt.savefig(utils.format_filename(workdir,
                                          'residmap_excess',
                                          prefix=[prefix],
    ...
    def make_tsmap_plots(self, maps, roi=None, **kwargs):
        ...
        prefix = maps['name']
        fig = plt.figure(figsize=figsize)
        maps['sqrt_ts'].data = gaussian_filter(maps['sqrt_ts'].data,4.0)
        ...
        #       zoom=zoom)
        p.plot(vmin=0, vmax=1.0, levels=sigma_levels,
               cb_label='Sqrt(TS) [$\sigma$]', interpolation='gaussian',
               zoom=zoom)
        fig.tight_layout(pad=3.0)
        plt.savefig(utils.format_filename(workdir,
                                          '%s_sqrt_ts' % suffix,
