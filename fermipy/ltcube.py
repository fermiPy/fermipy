...
def fill_livetime_hist(skydir, tab_sc, tab_gti, zmax, costh_edges):
    ...
    positions given by ``skydir``.

    Parameters
    ----------
    costh_edges : `~numpy.ndarray`
        Incidence angle bin edges.
    """
    ...
    sc_t0 = np.array(tab_sc['START'])
    sc_t1 = np.array(tab_sc['STOP'])
    sc_live = np.array(tab_sc['LIVETIME'])
    sc_lfrac = sc_live/(sc_t1-sc_t0)

    sc_ra_rad = np.radians(tab_sc['RA_SCZ'])
    sc_dec_rad = np.radians(tab_sc['DEC_SCZ'])
    zn_ra_rad = np.radians(tab_sc['RA_ZENITH'])
    zn_dec_rad = np.radians(tab_sc['DEC_ZENITH'])
    
    tab_gti_t0 = np.array(tab_gti['START'])
    tab_gti_t1 = np.array(tab_gti['STOP'])
    ...
    for i, t in enumerate(skydir):

        cos_sep = separation_cos_angle(sc_ra_rad, sc_dec_rad,
                                       t.ra.rad,t.dec.rad)
        cos_zn = separation_cos_angle(zn_ra_rad, zn_dec_rad,
                                      t.ra.rad,t.dec.rad)
        m = m0 & (cos_zn > cos_zmax) & (cos_sep > 0.0)
        lt[:,i] = np.histogram(cos_sep[m], bins=costh_edges,
                               weights=sc_live[m])[0]
        lt_wt[:,i] = np.histogram(cos_sep[m], bins=costh_edges,
                                  weights=sc_live[m]*sc_lfrac[m])[0]
...
class LTCube(HpxMap):
    ...
    def create_from_gti(skydir, tab_sc, tab_gti, zmax, **kwargs):
        ...
        lt, lt_wt = fill_livetime_hist(hpx_skydir[m], tab_sc, tab_gti, zmax, cth_edges)
        map_lt.data[:,m] = lt
        map_lt_wt.data[:,m] = lt_wt
        
        hpx2 = HPX(2**6, True, 'CEL', ebins=cth_edges)

        ltc = LTCube(np.zeros((len(cth_edges)-1,hpx2.npix)),hpx2,cth_edges)
    ...
    def create_skydir_ltcube(self, skydir, tab_sc, tab_gti, zmax):
        ...
        skydir = SkyCoord(np.array([skydir.ra.deg]),np.array([skydir.dec.deg]),unit='deg')
